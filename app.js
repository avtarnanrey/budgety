...
var controller = (function(BgtCtr, UICtr){
		...
		var updateBudget = function(){
			...
			BgtCtr.calculateBudget();

			// 2. Return the Budget
			var budget = BgtCtr.getBudget();
		...
		var updatePercentages = function(){
			...
			BgtCtr.calculatePercentages();

			// 2. Read Percentages from the budget control
			var percentages = BgtCtr.getPercentage();

			// 3. Update the UI with new percentage
			UICtr.displayPercentage(percentages);
		...
		var ctrlAddItem = function(){
			...
			inputs = UICtr.getInput();

			if(inputs.description !== "" && !isNaN(inputs.value) && inputs.value > 0){
				// 2. Add items to the budget controller
				newItem = BgtCtr.addData(inputs.type, inputs.description, inputs.value);

				// 3. Add Item to UI
				UICtr.addListItems(newItem, inputs.type);
		...
		var ctrlDeleteItem = function(event){
			...
			if(itemID){
				...
				// 2. Delete item from data struture
				BgtCtr.deleteItem(type, ID);
